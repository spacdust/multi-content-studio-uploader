...
export default function AccountSwitcherPopover({
  ...
  const isCurrentAnyActive =
    currentAccData.tiktok_active || currentAccData.instagram_active || currentAccData.facebook_active;
    ...
    <div className="relative" ref={accountDropdownRef}>
      ...
      >
        ...
        <div className="relative flex-shrink-0">
          ...
          <span
            className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-zinc-900 ${
              isCurrentAnyActive ? 'bg-emerald-400 shadow-xs shadow-emerald-400/50' : 'bg-amber-400'
            }`}
          />
        </div>
        ...
        <div className="flex flex-col text-left min-w-0">
          ...
          <span className="text-[10px] font-mono text-cyan-400/90 truncate max-w-[130px] sm:max-w-[170px]">
            {currentAccData.tiktok_profile?.username
              ...
              : (() => {
                  const activeList = [];
                  if (currentAccData.tiktok_active) activeList.push('TT');
                  if (currentAccData.instagram_active) activeList.push('IG');
                  if (currentAccData.facebook_active) activeList.push('FB');
                  return activeList.length > 0 ? `● ${activeList.join(' · ')} Aktif` : '○ Belum Login';
                })()}
          </span>
        </div>

        {/* Platform Connection Status Chips (TT | IG | FB) */}
        <div className="hidden sm:flex items-center gap-1.5 px-2 py-0.5 bg-zinc-950/60 rounded-md border border-zinc-800/80 text-[10px] font-mono text-zinc-400">
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${currentAccData.instagram_active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
          <span>IG</span>
          <span className="text-zinc-700">|</span>
          <span className={`w-1.5 h-1.5 rounded-full ${currentAccData.facebook_active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
          <span>FB</span>
        </div>
        ...
        <div className="absolute top-full left-0 mt-2 z-50 w-80 sm:w-96 rounded-2xl bg-zinc-950/95 backdrop-blur-xl border border-zinc-800 shadow-2xl p-2 animate-fadeIn flex flex-col gap-1 ring-1 ring-white/5">
          ...
          <div className="flex flex-col gap-1 max-h-72 overflow-y-auto py-1 custom-scrollbar">
            {accounts.map((acc) => {
              ...
              const isAccActive = acc.tiktok_active || acc.instagram_active || acc.facebook_active;
              return (
                <div
                  key={acc.name}
                  onClick={() => {
                ...
                >
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      <span
                        className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-zinc-950 ${
                          isAccActive ? 'bg-emerald-400' : 'bg-amber-400'
                        }`}
                      />
                    </div>
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] font-mono text-zinc-400 truncate">
                        {acc.tiktok_profile?.username ? (
                          ...
                          const list = [];
                          if (acc.tiktok_active) list.push('TT');
                          if (acc.instagram_active) list.push('IG');
                          if (acc.facebook_active) list.push('FB');
                          return <span>{list.length > 0 ? `${list.join(' · ')} Siap` : 'Belum Login'}</span>;
                        })()}
                  ...
                  {/* Right: Active Badge or Platform Status (TT | IG | FB) */}
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {isSelected ? (
                      ...
                      <div className="flex items-center gap-1 text-[10px] font-mono text-zinc-500 group-hover:text-zinc-300">
                        ...
                        <span>TT</span>
                        <span className="text-zinc-700">|</span>
                        <span className={`w-1.5 h-1.5 rounded-full ${acc.instagram_active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                        <span>IG</span>
                        <span className="text-zinc-700">|</span>
                        <span className={`w-1.5 h-1.5 rounded-full ${acc.facebook_active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                        <span>FB</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="pt-2 border-t border-zinc-800/80 flex flex-col gap-1.5">
            ...
            {/* 3 Shortcut Platform Browser Buttons */}
            <div className="grid grid-cols-3 gap-1.5 pt-0.5">
              {/* TikTok Studio Shortcut */}
              <button
                type="button"
                onClick={() => {
                  setIsAccountDropdownOpen(false);
                ...
                className="px-2 py-1.5 rounded-xl bg-cyan-950/30 hover:bg-cyan-950/50 border border-cyan-800/40 text-[10px] font-semibold text-cyan-300 flex items-center justify-between transition group/tt"
                title="Buka TikTok Studio Browser"
              >
                <span className="flex items-center gap-1 truncate">
                  <ExternalLink className="w-3 h-3 text-cyan-400 flex-shrink-0 group-hover/tt:scale-110 transition-transform" />
                  <span className="truncate">TikTok</span>
                </span>
                <ArrowUpRight className="w-2.5 h-2.5 text-cyan-400 flex-shrink-0" />
              </button>

              {/* Instagram Web Shortcut */}
              <button
                type="button"
                onClick={() => {
                  setIsAccountDropdownOpen(false);
                  handleOpenInstagramBrowser(selectedAccount);
                }}
                className="px-2 py-1.5 rounded-xl bg-pink-950/30 hover:bg-pink-950/50 border border-pink-800/40 text-[10px] font-semibold text-pink-300 flex items-center justify-between transition group/ig"
                title="Buka Instagram Web Browser"
              >
                <span className="flex items-center gap-1 truncate">
                  <ExternalLink className="w-3 h-3 text-pink-400 flex-shrink-0 group-hover/ig:scale-110 transition-transform" />
                  <span className="truncate">Instagram</span>
                </span>
                <ArrowUpRight className="w-2.5 h-2.5 text-pink-400 flex-shrink-0" />
              </button>

              {/* Facebook Fanspage Shortcut */}
              <button
                type="button"
                onClick={() => {
                  setIsAccountDropdownOpen(false);
                  if (handleOpenFacebookBrowser) handleOpenFacebookBrowser(selectedAccount);
                }}
                className="px-2 py-1.5 rounded-xl bg-blue-950/30 hover:bg-blue-950/50 border border-blue-800/40 text-[10px] font-semibold text-blue-300 flex items-center justify-between transition group/fb"
                title="Buka Facebook Fanspage Browser"
              >
                <span className="flex items-center gap-1 truncate">
                  <ExternalLink className="w-3 h-3 text-blue-400 flex-shrink-0 group-hover/fb:scale-110 transition-transform" />
                  <span className="truncate">Facebook</span>
                </span>
                <ArrowUpRight className="w-2.5 h-2.5 text-blue-400 flex-shrink-0" />
              </button>
            </div>
          </div>
        </div>
